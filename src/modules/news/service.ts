...
const NEWS_API_URL = process.env.NEWS_API_URL as string;
...
export default class NewsService {
  static async fetchPokemonNews(
    ...
    const response = await fetch(
      `${NEWS_API_URL}/v2/everything?qInTitle=pokemon&language=en&pageSize=${pageSize}&page=${page}&sortBy=publishedAt&apiKey=${NEWS_API_KEY}`
    );
